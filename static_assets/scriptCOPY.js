function searchRecipe() {
    var searchTerm = document.getElementById('searchInput').value.trim(); // Get the value from the input field with id 'searchInput' and remove any leading/trailing whitespace
    if (searchTerm !== '') {
...
function displayRecipe(recipeData) {
    ...
    document.getElementById('name').innerText = recipeData[0];  // Set the inner text of the element with id 'name' to the 1st item in the recipeData array
    document.getElementById('description').innerText = recipeData[1]; // Set the inner text of the element with id 'description' to the 2nd item in the recipeData array
    document.getElementById('steps').innerText = recipeData[2]; // Set the inner text of the element with id 'steps' to the 3rd item in the recipeData array
    document.getElementById('ingredients').innerText = recipeData[3]['Ingredients'];  // Set the inner text of the element with id 'ingredients' to the 'Ingredients' property of the 4th item in the recipeData array
}
...
async function fetchRecipe(searchTerm) { // async function can execute tasks that takes time (like fetching data from a server) without blocking the main execution thread of the program.
    var url = 'http://127.0.0.1:8000/recipe/' + encodeURIComponent(searchTerm);
      // Construct the URL for the API endpoint. encodeURIComponent() ensures that special characters in the searchTerm are properly encoded for inclusion in a URL.
